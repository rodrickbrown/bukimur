...
if game.initalizeGame() {
    ...
    while(true) { // Main input loop for game logic
        ...
        inputData = player.input().componentsSeparatedByString(" ")
        for tileNumber in inputData {
            let tn = tileNumber.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceAndNewlineCharacterSet()) // remove \n from string.
            let pos = String(tn).toInt()
            if (pos > 13 || pos < 0) { continue }  // tiles are available in slots 0-13 only
            tiles.append(player.rack[pos!])
        }
        // println("Validating set.....")
        println(tiles)
        if Sets(sets: tiles).validateSetRun() == false || Sets(sets: tiles).validateSetRun() == false {
            println("Invalid run sorry!")
        }
        else {
            println("Valid Set run awesome!!!")
        }
        // tiles = [] // We need to empty the list for the next run
    }
